...
if (stim_vals(end) >= stim_vals(1)) % This is for luminance increases
   ...
   anon_gausscdf = @(params, x) -1*normcdf(x,params(1), params(2)) + 1; %normal cumulative gaussian function
end
...
% fit the whole curve to the gaussian CDF function
%[fit_params,resnorm,resid,ef,output] = lsqcurvefit(anon_gausscdf, [mu0, sigma0], stim_t, stim_vals, lb, ub, opts);
...
bl = length(between); 
range = (1:ceil(bl*2/3)) + floor(bl/3) + between(1); % take the later 2/3 of the luminance change
...
mean_err = sum(abs(resid))./length(stim_t);
if (ef <= 0 || mean_err > .1)
    disp(output.message);
    figure; plot(stim_t, stim_vals, 'k', stim_t, fit_trace, 'r');
    xlabel('Time'); ylabel('Value'); title('Proposed fit');
    keyboard;
end
if(exist('plot_ah'))
